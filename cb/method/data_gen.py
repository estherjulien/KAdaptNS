...
def data_gen_fun_max(K, env, att_series, problem_type="test", time_limit=5*60, perc_label=0.05, normalized=False, dg_inst=None):
    ...
    os.makedirs("cb/data/train_data/inst_results", exist_ok=True)
    if dg_inst is None:
        dg_inst = env.inst_num
    with open(f"cb/data/train_data/inst_results/data_results_{problem_type}_"
              f"{dg_inst}.pkl", "wb") as handle:
        pickle.dump(results, handle)

    # save information
    os.makedirs("cb/data/train_data/inst_results_md", exist_ok=True)
    pd.Series([len(input_data["no"]), level, rt_mean, num_start_nodes, tot_scens_init, time_per_node, *num_runs_info, runtime],
              ...
              dtype=float).to_pickle(f"cb/data/train_data/inst_results_md/"
