...
def algorithm(K, env, att_series=None, max_level=None, success_model_name=None, time_limit=30 * 60, print_info=True,
              problem_type="test", thresh=None, num_runs=5, sc_pre="dive", sc_min_max=0):
    ...
    # initialize N_set
    N_set, tau, scen_all, att_all, att_all_k, zeta_init, theta_start = fill_subsets(K, env, att_series, x_static, gp_env)
    ...
    if sc_pre == "dive":
        ...
        for i in range(num_runs):
            theta, tot_scens, new_nodes, num_in_subset, zeta = random_pass(K, env, tau, gp_env)
    ...
    while (N_set or N_set_trash) and time.time() - start_time < time_limit:
        ...
        if len(full_list) == 0:
            ...
            if max_level is not None and sum([len(t) for t in tau.values()]) - K >= max_level:
                ...
                k_att_sel = {k: np.array([att_all_k[p][k] for p in placement[k]]) for k in range(K)}
                tau_att = {k: np.hstack([att_all[placement[k]], k_att_sel[k]]) for k in range(K)}
...
def fill_subsets(K, env, att_series, x_static, gp_env, progress=False):
    # Initialize
    start_time = time.time()
    ...
    xi_init, att_init, att_init_k = init_scen(K, env, att_series, stat_model, det_model, x_static, gp_env)

    N_set = [{k: [] for k in range(K)}]
    N_set[0][0].append(0)
