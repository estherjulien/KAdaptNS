...
import os
"""
Code for running generating training data for the shortest path problem
...
def data_gen_fun(K, env, att_series, problem_type="test", time_limit=5*60, perc_label=0.05, normalized=False, dg_inst=None):
    # Initialize
    ...
    tau, tau_att, new_scen, theta_start, zeta_init = fill_subsets(K, env, att_series, gp_env)
    ...
    os.makedirs("src/sp/data/train_data/inst_results", exist_ok=True)
    if dg_inst is None:
        dg_inst = env.inst_num
    with open(f"src/sp/data/train_data/inst_results/data_results_{problem_type}_"
              f"{dg_inst}.pkl", "wb") as handle:
        pickle.dump(results, handle)

    # save information
    os.makedirs("src/sp/data/train_data/inst_results_md", exist_ok=True)
    pd.Series([len(input_data), level, rt_mean, num_start_nodes, tot_scens_init, time_per_node, *num_runs_info, runtime],
              ...
              dtype=float).to_pickle(f"src/sp/data/train_data/inst_results_md/"
...
def fill_subsets(K, env, att_series, gp_env):
    # Initialize
    start_time = time.time()
    ...
    tau, tau_att = init_scen(K, env, att_series, det_model, gp_env)
...
def make_upper_tree(K, env, att_series, tau, tau_att, theta_init, theta_start, zeta_init,
                    ...
                    max_depth=5, time_limit=5*60, rt_mean=10):
    rng = np.random.RandomState(env.inst_num)
    gp_env = gp.Env()
